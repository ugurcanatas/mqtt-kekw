...
const convertKeepAliveToHex = ({ seconds = 0, hours = 0, minutes = 0, }) => {
    ...
    let msb = parseInt(total.slice(0, 2), 16);
    let lsb = parseInt(total.slice(2, total.length), 16);
    return [msb, lsb];
};
const fieldWithSize = (payload) => {
    console.time("fieldWithSize");
 ...
 * @desc [description]
 */
class kekwClient extends EventEmitter__default["default"] {
    ...
    timeout = 5000, } = {}, onFailed) {
        ...
        this.connectionUp = ({ flags = {
            ...
            minutes: 0,
            seconds: 0,
        }, clientID = `mqttClient_${(Math.random() + 1).toString(16).substring(8)}`, will = {
            willTopic: "topic",
            willMessage: "message",
        }, } = {}) => {
            var _a;
        ...
        this.client = new net__default["default"].Socket();
...
const client = new kekwClient({}, (message) => {
    console.log("FAILED CALLBACK", message);
});
let packetIdentifier = [];
client.on("ready", () => {
...
client.on("close", (hadError) => {
    console.log("Client Connection Closed !!", hadError);
});
client.on("end", () => {
    console.log("Client Connection End !!");
});
client.on("timeout", () => {
    console.log("Client Connection Timedout !!");
});
client.on("pingresp", (response) => {
    console.log("Client Ping Response", response);
});
client.on("puback", (payload) => {
    console.log("Client Puback Response", payload);
});
client.on("pubrec", (payload) => {
    console.log("Client Pubrec Response", payload);
});
...
client.on("received", (payload) => {
    console.log("Client Received A Message", payload);
});
client.on("connectionAccepted", ({ returnCode, message }) => {
    console.log("Connection Accepted Return Code", returnCode);
    console.log("Connection Accepted Message", message);
});
client.on("connectionRefused", ({ returnCode, message }) => {
    console.log("Connection Refused Return Code", returnCode);
    console.log("Connection Refused Message", message);
});
const rl = readline__namespace.createInterface({
    input: process.stdin,
    output: process.stdout,
});
let recursiveLine = () => rl.question("Send A Message ", (answer) => {
    ...
    switch (answer) {
        ...
        case "UNSUB":
            ...
            break;
        case "PUB":
            // kekw_client.sendPacket({
            //   controlPacketType: CONTROL_PACKET_TYPES.PUBLISH,
            //   packetType: {
            //     topic: "topic1",
            //     message:
            //       "topic1 message send from node client. Long Long Long messageeeee",
            //     dupFlag: 0,
            //     retain: 0,
            //   },
            // });
